# Getting Started with bot_army_media_ingestion

This guide walks you through setting up Media Ingestion Bot for local development.
...
DATABASE_NAME=bot_army_media_ingestion_dev
```
...
Media Ingestion Bot uses the **BotArmy.GenBot** harness to run skills. Skills are discrete, reusable units of work triggered via NATS messages.

### Creating Your First Skill

1. **Start with the template** — `lib/bot_army_media_ingestion/skills/example.ex` is a fully documented template
2. **Copy and rename** it for your skill:
   ```bash
   cp lib/bot_army_media_ingestion/skills/example.ex lib/bot_army_media_ingestion/skills/my_skill.ex
   ```
...
```elixir
defmodule BotArmyMediaIngestion.Skills.Classify do
  use BotArmy.Skill

  def name, do: :classify
  def description, do: "Classifies text into categories"
  def nats_triggers, do: ["bot.bot_army_media_ingestion.command.classify"]
...
```elixir
# In lib/bot_army_media_ingestion/application.ex or similar

defmodule BotArmyMediaIngestion.MyBot do
  use BotArmy.GenBot,
    skills: [
      BotArmyMediaIngestion.Skills.Classify,
      BotArmyMediaIngestion.Skills.ExtractEntities
    ],
    bot_id: :my_bot
end
```
...
```bash
# Make your changes
edit lib/bot_army_media_ingestion/skills/my_skill.ex
...
releases: [
  media_ingestion_bot: [
    applications: [bot_army_media_ingestion: :permanent]
  ]
]
```

This creates a release named `media_ingestion_bot` that is deployed to `/opt/ergon/releases/media_ingestion_bot/` on the server.
...
3. Extracts and deploys to `/opt/ergon/releases/media_ingestion_bot/`
