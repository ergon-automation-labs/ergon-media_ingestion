pipeline {
  ...
  environment {
    BOT_NAME = 'media_ingestion_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-media_ingestion"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/media_ingestion_bot/bin/media_ingestion_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyMediaIngestion.Release.create()'"
          $RELEASE_BIN eval 'BotArmyMediaIngestion.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyMediaIngestion.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyMediaIngestion.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_media_ingestion/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_media_ingestion/releases/start_erl.data)
        fi
